...
HttpResponse::HttpResponse() : httpVersion("HTTP/1.1"), bytesSent(0), headersSent(false), headersSize(0), headersBytesSent(0) {
	init();
}
...
HttpResponse::HttpResponse(const HttpResponse& other): httpVersion(other.httpVersion), statusCode(other.statusCode),
	...
	bytesSent(other.bytesSent), headersSent(other.headersSent), headersSize(other.headersSize), headersBytesSent(other.headersBytesSent)
{}
...
{
	return httpVersion + " " + std::to_string(statusCode) + " " + statusMessage;
}
...
std::vector<char> HttpResponse::assembleResponse() {
	...
	if (getBodySource() == BODY_BUFFER) {
        ...
        char buffer[4096];
        fileBody.read(buffer, sizeof(buffer));
		std::streamsize bytesRead = fileBody.gcount();
        if (bytesRead <= 0)
            return std::vector<char>();
        return std::vector<char>(buffer, buffer + bytesRead);
    }
    return std::vector<char>();
}
