...
void CgiRequest::setHttpEnvs(void) {
	...
	for ( ; it != headers_map.end(); ++it) {
		if (compare_header(it->first, "HOST") ||
				compare_header(it->first, "CONETENT-LENGTH")||
				compare_header(it->first, "TRANSFER-ENCODING"))
			continue;	
...
void CgiRequest::setScriptInfoEnv() {
	m_env_vec.push_back("SCRIPT_FILENAME=" + m_request._routeResult.targetPath);
