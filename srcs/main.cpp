
#include "ConnectionManager.hpp"
#include "webserver.hpp"

void printRouteTree(RouteNode &route, int tabNum) {

  std::cout << std::string(tabNum, '\t')
            << "segment name = " << route.segmentName << "\n";
  if (route.config)
    std::cout << std::string(tabNum, '\t')
              << " config = " << route.config->getRedirection().second << "\n";
  if (route.children.size() == 0)
    return;
  for (std::map<std::string, RouteNode *>::const_iterator it =
           route.children.begin();
       it != route.children.end(); ++it) {
    // std::cout << std::string(tabNum, '\t') << "key = " << it->first << "\n";
    printRouteTree(*it->second, ++tabNum);
  }
}

void handle_request(Config::ServerRange range, const std::string &str) {
  HttpRequest request(range, Server::IPort());
  request.parse(std::vector<char>(str.begin(), str.end()));
  if (request.getCurrentState() == FINISHED) {
    std::cout
        << "===================this is routing phase=======================\n";

    RouteManager m;
    RouteResult result = m.processRequest(request);
    std::cout << "status Code = " << result.statusCode << "\n";
    if (result.action != ACTION_ERROR) {
      std::cout << "target path = " << result.targetPath << "\n";
		system(std::string("cat " + result.targetPath).c_str());
      RouteManager::printRouteAction(result.action);
      //
      // CgiRequest cgi(request);
      // cgi.printEnv();
    } else {
      HttpRequest::printHttpStatus(result.statusCode);
      std::cout << "error\n";
    }
  } else {
    HttpRequest::printHttpStatus(request.getStatusCode());
  }
}

int main(int argc, char **argv) {
  if (argc != 2) {
    std::cerr << "./webserver configfile.conf";
    return (1);
  }
  try {
    std::vector<token> tokens = lexer::tokenizeFile(argv[1]);

    ParseConfig parser(tokens);
    try {
      Config conf = parser.parse();
      ConnectionManager manager(conf);

      // Config::ServerMultiMap& mymap = conf.m_iport_server;
      // for (Config::ServerMultiMap::iterator it = mymap.begin(); it !=
      // mymap.end(); ++it) { 	it->second.buildRouteTree();
      // }
      // buildRouteTree((++mymap.begin())->second);
      //
      const Config::ServerMultiMap &map = conf.m_iport_server;
      Config::ServerRange range = map.equal_range(map.begin()->first);

      std::string request_str1 =
          "GET /var/www/html/ HTTP/1.1\r\nHOST: server1\r\n\r\n";
      std::string request_str2 =
          "GET /test/autoindex2/ HTTP/1.1\r\nHOST: server2\r\n\r\n";
      handle_request(range, request_str1);
      handle_request(range, request_str2);

    } catch (const ParseConfig::ConfigExcept &e) {
      std::cerr << e.what() << "\n";
    }
  } catch (const std::exception &e) {
    std::cerr << e.what() << "\n";
    return (1);
  }

  return (0);
}
